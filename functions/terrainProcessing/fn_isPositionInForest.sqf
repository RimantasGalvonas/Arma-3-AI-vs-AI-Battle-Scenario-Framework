params ["_position", ["_strictMode", true]];

if (_strictMode) exitWith { // Mode that checks if we are deep enough in the forest.
    private _depth = 100;
...
switch (toLower worldName) do
{
    case "spe_mortain": { _depth = 50; }; // Lots of tree rows that better not be counted as forest.
    case "enoch": { _depth = 50; }; // Same
};
